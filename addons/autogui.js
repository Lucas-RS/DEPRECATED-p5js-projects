class AutoGUI extends dat.GUI {
  ...
  enablePresets(defaultPresets, userPresetsStorageName) {
    ...
    this.userPresets =
      JSON.parse(localStorage.getItem(this.userPresetsStorageName)) || {};
    ...
    this.controllers["presetSelector"] = this.add(
      this.presetControllers,
      "presetSelector",
      ["Default", ...Object.getOwnPropertyNames(this.presets)]
    ).name("PRESET:");
    this.controllers["presetSave"] = this.add(
      this.presetControllers,
      "presetSave"
    ).name("Save New Preset");
    this.controllers["presetSelector"].__li.style.cssText = "border: 0";
    this.controllers["presetSelector"].__select.style.cssText =
      "margin-top: 3px;";
    this.controllers["presetSave"].__li.style.cssText =
      "background-color: #2fa1d6; border: 0; padding: 0;";
    this.controllers["presetSave"].__li.children["0"].children[
      "0"
    ].style.cssText =
      "background-color: #2fa1d6; width: 100%; text-align: center; text-shadow: none;";
    ...
    if (Object.keys(newPreset).length > 0) {
      ...
      while (true) {
        if (presetName === null) {
          ...
          presetName = prompt(
            "Cannot overwrite a default preset.\nChoose a different name: "
          );
        } else if (
          presetName in this.userPresets &&
          !confirm(
            "This will overwrite your previously saved preset which already has this name.\nDo you wish to continue?"
          )
        ) {
          ...
          let currentPresets = [
            "Default",
            ...Object.getOwnPropertyNames(this.presets)
          ];
          let newHTML;
      ...
      alert(
        "You have not changed any settings, what is the use in an empty preset?"
      );
    }
  }
  ...
  addMenuFolderSwitch(select, parentFolder, ignoreFolders = []) {
    let that = this;
  ...
  sticky(controller, bgColor = "#1a1a1a") {
    ...
    this.controllers[controller].__li.style.top =
      this.stickiedListItems * 28 + "px";
